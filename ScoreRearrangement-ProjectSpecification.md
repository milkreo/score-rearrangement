...
- Pseudo Piano Solo synthesis: **union-grid merge** (`merge_violin_into_R`, revised 2026-06-02). The piano R and the violin are overlaid on the union of their note onsets/offsets; each resulting slice becomes one event holding whatever pitches sound during it (piano-R + violin, deduplicated), or a rest if silent. Bars containing `<voice>` markers (multi-voice within one hand) keep the piano R unchanged, since onset math is ambiguous across concurrent voices.
  - **Why union-grid replaced the original "piano rhythm is the master clock" merge:** the old version folded every violin pitch overlapping a single piano-R window into one chord, so a whole-bar piano rest (or any long piano note) collapsed an entire bar of melody — e.g. Do-Re-Mi-Fa — into a single chord struck on beat 1, destroying the melodic rhythm. The union grid keeps each violin note at its own onset/duration; when the piano R is silent (the common case in these duets, where the right hand often rests under the violin), the melody is reproduced verbatim.
  - **Limitation:** a pitch held across a *foreign* onset (e.g. a sustained violin note while the piano R re-strikes underneath) is re-articulated at that boundary rather than tied (one event per slice). This deliberately keeps repeated same-pitch notes distinct — a tie would wrongly fuse "Do Do" into one note — at the cost of a minor re-articulation artifact in densely polyphonic bars.
  - **Train/test consistency:** `pairs_duet.jsonl` and the trained checkpoint (`best.pt`, 2026-06-01) were built with the *old* collapse merge. The new merge is closer to real piano-solo notation, so it widens the gap against the existing training data; for fully consistent results, `pairs_duet.jsonl` should be regenerated with the union-grid merge and the model retrained. Until then, inference inputs from `make_pseudo_solo.py` (which uses the new merge) differ rhythmically from what the model trained on.
...
**Real-inference verification (2026-06-02).** Ran the trained checkpoint end-to-end on a synthesized pseudo-solo (`Phase06/make_pseudo_solo.py --index 0`, 44 bars). Both tasks now produce structurally valid multi-staff MusicXML (melody → 1-part violin; duet → 3-part Violin+PianoR+PianoL, matching the ground-truth duet's part/bar structure). **Bug fixed in `infer_duet.py`:** segment concatenation prepended an extra `bar` to each decoded segment, but segments already begin with their own `bar`; the resulting empty bar crashed piano detokenization (`to_ST`/`tokens_to_score` → "'R' is not in list") and silently inserted spurious empty bars in melody output. Concatenation now goes through `append_segment`, which relies on each segment's own leading `bar`. The smoke-test missed this because it validated single, clean dataset targets — never the multi-segment concatenation real inference performs.

- **Status:** SCRIPT COMPLETE; **training DONE (2026-06-01).** Model trained from scratch (Path A) on a lab GPU. Best checkpoint `data/checkpoints_duet/best.pt` saved at **epoch 70 (val_loss ≈ 0.758)**; run reached ~71 epochs before early stopping (patience 10). Real end-to-end inference now in progress (see §6.4 inference CLI). Code: `Phase06/infer_duet.py`.

**Resolved: training compute.** Earlier blocker (the original dev machine had no CUDA GPU; CPU training of 146,732 duet pairs was impractical) was resolved by training on a **lab GPU machine** (option (a)). The resulting `best.pt` (~4.9 MB, 0.3 M-param model) is fully portable: inference needs no GPU — `infer_duet.py` auto-detects device and falls back to CPU — so it can be exercised end-to-end on any machine with `torch` + `music21`. Pseudo-solo test inputs for inference are synthesized from `tokens_duet/` duets via `Phase06/make_pseudo_solo.py` (Phase 6.4 helper).
...
score-rearrangement/
    ...
    Phase06/
        ...
        make_pseudo_solo.py      synthesize pseudo-solo MXL + ground-truth from a duet [Phase 6.4]
